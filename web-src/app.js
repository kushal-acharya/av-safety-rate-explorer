...
function plotRates(rows) {
  const w=660,h=Math.max(255,rows.length*43+65),l=state.companies.length===1?54:132,r=88,t=28,b=40;
  ...
  for(let i=0;i<5;i++) { const value=log?10**(Math.log10(min)+(Math.log10(max)-Math.log10(min))*i/4):max*i/4,xx=x(value); svg+=`<line x1="${xx}" x2="${xx}" y1="${t-10}" y2="${h-b+3}" stroke="#edf1e7"/><text x="${xx}" y="${h-17}" text-anchor="middle" fill="#97a18f" font-size="9">${rateFmt(value)}</text>`; }
  ...
  svg+=`<text x="${(w+l-r)/2}" y="${h-1}" text-anchor="middle" font-size="8" fill="#97a18f">DISENGAGEMENTS ${unitLabel().toUpperCase()}${log?' · LOG SCALE':''}</text></svg>`;
  return svg;
}
function renderRates() {
  ...
  $('view-rates').innerHTML=heading('01','Rate explorer','Follow the rate. Keep the uncertainty in view.',`${rows.length} reporting groups`)+`
    ...
    <div class="chart-frame">${plotRates(g)}</div><div class="chart-legend"><span><i class="legend-point"></i>Estimated rate</span><span><i class="legend-line"></i>${conf()} confidence interval</span><span style="color:#c77a50">● Zero-event upper bound</span></div>
...
function renderPlannerOutput() {
  try {
    ...
    const w=650,h=245,l=48,right=22,t=20,b=42,max=result.total_miles*2,xx=m=>l+m/max*(w-l-right),yy=p=>h-b-p*(h-t-b);
    ...
    for(let i=0;i<=4;i++){const p=i/4,y=yy(p);chart+=`<line x1="${l}" x2="${w-right}" y1="${y}" y2="${y}" stroke="#edf1e7"/><text x="${l-12}" y="${y+3}" text-anchor="end" font-size="9" fill="#8b9a83">${p*100}%</text><text x="${xx(max*p)}" y="${h-20}" text-anchor="middle" font-size="9" fill="#8b9a83">${compact(max*p)}</text>`;}
